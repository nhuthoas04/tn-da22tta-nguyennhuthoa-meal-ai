...
export class InventoryService {
    ...
    async findAll(userId: string, expiringSoon?: boolean, category?: string) {
        ...
        const data = items.map((item) => {
            const daysLeft = item.expirationDate ? this.getDaysLeft(item.expirationDate) : null;
            return {
                id: item.id,
                ingredient: {
    ...
    async create(userId: string, dto: {
        ...
        const daysLeft = saved.expirationDate ? this.getDaysLeft(saved.expirationDate) : null;
     ...
     */
    async update(userId: string, id: string, dto: { quantity?: number; expirationDate?: string }) {
