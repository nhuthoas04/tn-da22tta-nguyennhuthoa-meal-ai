...
export default function AdminPendingClient() {
  ...
  const handleApprove = async (id: string) => {
    ...
    const recipe = pendingRecipes.find((r) => r.id === id);
    if (!recipe) return;
    ...
    try {
      ...
      console.error(error);
      toast.error('Không thể xử lý công thức.');
      await fetchPendingRecipes();
    } finally {
      setApprovingId(null);
    }
  ...
  const handleReject = async () => {
    ...
    const recipe = pendingRecipes.find((r) => r.id === rejectingId);
    if (!recipe) return;
    const status = recipe.status?.toLowerCase();
    if (status !== 'pending') {
      toast.error('Công thức này không còn ở trạng thái chờ duyệt. Danh sách sẽ được tải lại.');
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">
          {pendingRecipes.length > 0
            ? `${pendingRecipes.length} bài đang chờ xem xét`
