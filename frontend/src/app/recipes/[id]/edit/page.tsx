...
export default function EditRecipePage() {
  ...
  const loadRecipeDetails = async () => {
    try {
      ...
      // In the database, the submittedBy column maps to user's id. We can verify if they match.
      // If recipe is not submitted by user (or they are not admin), we should prevent edits.
      // Wait, we can double-check on backend as well, but checking on frontend is good.
      ...
      setForm({
        ...
        fat: recipe.fat ? Number(recipe.fat) : 0,
        tags: recipe.tags ? recipe.tags.join(', ') : '',
        mealType: recipe.mealType || [],
        cuisineRegion: recipe.cuisineRegion || '',
        estimatedCost: recipe.estimatedCost ? Number(recipe.estimatedCost) : 0,
      });
  ...
  if (!user) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 mb-4">Bạn cần đăng nhập để chỉnh sửa công thức</p>
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm animate-pulse h-96" />
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className="text-center py-16 space-y-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Cập nhật thành công!</h2>
        <p className="text-gray-500 max-w-md mx-auto">
          Công thức của bạn đã được cập nhật thành công và đang chờ Admin duyệt lại.
        </p>
        <div className="flex justify-center gap-3 mt-6">
          <Link href="/profile/my-recipes" className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition">
            Quản lý bài viết
          </Link>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await recipesAPI.updateSubmission(id, data);
      setSubmitted(true);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3">
        <Link href="/profile/my-recipes" className="p-2 hover:bg-gray-100 rounded-lg transition">
          <HiArrowLeft className="text-xl text-gray-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa công thức 📝</h1>
          <p className="text-gray-500 mt-1">Cập nhật nội dung món ăn và gửi duyệt lại</p>
        </div>
      </div>

      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700">
        💡 Lưu ý: Khi lưu thay đổi, trạng thái công thức sẽ được cập nhật thành **Chờ duyệt** (Pending) để Admin kiểm duyệt lại nội dung.
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Khu vực ẩm thực</label>
        ...
        <div className="flex justify-end gap-3">
          <Link href="/profile/my-recipes" className="px-6 py-2.5 border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition">Hủy</Link>
          <button type="submit" disabled={saving} className="px-8 py-2.5 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition disabled:opacity-50">
